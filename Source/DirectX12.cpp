...
bool DirectX12::Initialize(HWND window)
{
	...
	VHR(CreateDXGIFactory2(DXGI_CREATE_FACTORY_DEBUG, IID_PPV_ARGS(&m_Factory)));
#else
	VHR(CreateDXGIFactory2(0, IID_PPV_ARGS(&m_Factory)));
#endif
	...
	// common GPU descriptor heaps
	for (uint32_t i = 0; i < 2; ++i)
	{
		D3D12_DESCRIPTOR_HEAP_DESC heapDesc = {};
		heapDesc.NumDescriptors = k_MaxGpuDescriptorCount;
		heapDesc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;
		heapDesc.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE;
		VHR(m_Device->CreateDescriptorHeap(&heapDesc, IID_PPV_ARGS(&m_GpuDescriptorHeap[i])));

		m_GpuDescriptorHeapCpuStart[i] = m_GpuDescriptorHeap[i]->GetCPUDescriptorHandleForHeapStart();
		m_GpuDescriptorHeapGpuStart[i] = m_GpuDescriptorHeap[i]->GetGPUDescriptorHandleForHeapStart();

		m_AllocatedGpuDescriptorCount = 0;
	}

	{	// common CPU descriptor heap
		D3D12_DESCRIPTOR_HEAP_DESC heapDesc = {};
		heapDesc.NumDescriptors = k_MaxCpuDescriptorCount;
		heapDesc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;
		heapDesc.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_NONE;
		VHR(m_Device->CreateDescriptorHeap(&heapDesc, IID_PPV_ARGS(&m_CpuDescriptorHeap)));

		m_CpuDescriptorHeapCpuStart = m_CpuDescriptorHeap->GetCPUDescriptorHandleForHeapStart();

		m_AllocatedCpuDescriptorCount = 0;
	}

	// common upload memory heaps
	for (uint32_t frameIdx = 0; frameIdx < 2; ++frameIdx)
	{
		VHR(m_Device->CreateCommittedResource(
			&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD), D3D12_HEAP_FLAG_NONE, &CD3DX12_RESOURCE_DESC::Buffer(k_UploadMemorySize),
			D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&m_UploadMemory[frameIdx])));

		VHR(m_UploadMemory[frameIdx]->Map(0, &CD3DX12_RANGE(0, 0), (void**)&m_UploadMemoryCpuAddr[frameIdx]));

		m_UploadMemoryGpuAddr[frameIdx] = m_UploadMemory[frameIdx]->GetGPUVirtualAddress();

		m_AllocatedUploadMemory = 0;
	}
...
void DirectX12::Present() const
{
	...
	// reset memory usage for new frame
	m_AllocatedGpuDescriptorCount = 0;
	m_AllocatedUploadMemory = 0;
}
...
void DirectX12::UploadAndReleaseAllIntermediateResources() const
{
	// Upload all the resources to the GPU
	m_CmdList->Close();
	m_CmdQueue->ExecuteCommandLists(1, (ID3D12CommandList**)&m_CmdList);
	Flush();

	// Release all intermediate resources after an upload is complete
	for (ID3D12Resource* res : m_IntermediateResources)
		res->Release();
